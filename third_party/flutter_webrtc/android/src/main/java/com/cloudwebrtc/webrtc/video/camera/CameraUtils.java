...
public class CameraUtils {
   ...
   * Requires CONTROL_AF_MODE_OFF and LENS_FOCUS_DISTANCE.
   */
  @RequiresApi(api = Build.VERSION_CODES.P)
  public void setFocusDistance(String trackId, double diopters, MethodChannel.Result result) {
    VideoCapturerInfo info = getUserMediaImpl.getCapturerInfo(trackId);
    if (info == null) {
      resultError("setFocusDistance", "Video capturer not found for id: " + trackId, result);
      return;
    }
    ...
    if (info.capturer instanceof Camera2Capturer) {
      ...
      try {
        ...
        resultError("setFocusDistance", "[FocusDistance] Failed to get `" + e.fieldName + "` from `" + e.className + "`", result);
        return;
      }
      ...
      try {
        ...
        captureRequestBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE, (float) diopters);
    ...
    resultError("setFocusDistance", "[FocusDistance] Only Camera2Capturer supported (API 28+)", result);
  }

  /**
   * Returns the maximum focus distance in diopters (closest focus) for the current camera.
   * LENS_INFO_MINIMUM_FOCUS_DISTANCE; 0 means fixed focus (infinity only).
   */
  @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
  public void getMaxFocusDistanceDiopters(String trackId, MethodChannel.Result result) {
    VideoCapturerInfo info = getUserMediaImpl.getCapturerInfo(trackId);
    if (info == null) {
      resultError("getMaxFocusDistanceDiopters", "Video capturer not found for id: " + trackId, result);
      return;
    }
    ...
    if (info.capturer instanceof Camera2Capturer) {
      ...
      try {
        ...
        resultError("getMaxFocusDistanceDiopters", "Failed to get session: " + e.fieldName, result);
        return;
      }
      ...
      try {
        ...
        // minFocusDistance is in diopters (closest focus); 0 = fixed focus at infinity
        result.success(minFocusDistance.doubleValue());
      } catch (CameraAccessException e) {
        throw new RuntimeException(e);
      }
      return;
